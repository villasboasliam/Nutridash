...
null>(null)
  ...
  const [selectedIndividualPDF, setSelectedIndividualPDF] = useState<File | 
null>(null)
...
null>(null)
  ...
  const [metricaParaExcluir, setMetricaParaExcluir] = useState<any | 
null>(null)
  ...
  const 
[densidadeCorporalNovoInput, setDensidadeCorporalNovoInput] = useState("")

  // ====== NOVO: protocolos extras e sexo normalizado ======
  const [protocoloDobras, setProtocoloDobras] = useState<ProtocoloKey>("pollock3")
  const [sexoAvaliacao, setSexoAvaliacao] = useState<string>((patient?.sexo ?? 
"feminino").toLowerCase())
  ...
  const [dobraSupraespinhal, setDobraSupraespinhal] = 
useState("") // reserva (variações)
...
cleanedValue.trim() === "" ? 0 : Number(cleanedValue)
  }
...
soma > 0 ? densidadePetroski7(soma, idade, sexo) : 0
      ...
      case "guedes": {
        ...
        dens = soma > 0 ? 
densidadeGuedes3(soma, idade, sexo) : 0
        break
      }
      case "durnin": {
        ...
        dens = soma > 0 ? 
densidadeDurnin4(soma, idade, sexo) : 0
        break
      }
      case "faulkner": {
...
nome: file.name,
      ...
      const statRef = doc(db, "nutricionistas", 
user.email, "estatisticas", "dietas")
      try {
        const statSnap = await getDoc(statRef)
        if (statSnap.exists()) {
          const atual = statSnap.data().totalDietasEnviadas || 
0
      ...
      setSubmitButtonColorClass("bg-green-500 
hover:bg-green-600")
      setTimeout(() => {
...
if (file) setSelectedPhoto(file)
  ...
  const handleUploadPhotos = async (event: React.FormEvent<HTMLFormElement>) => {
    event.preventDefault()
    if (!user?.email) { toast({ title: "Erro", description: "Usuário não autenticado." }); 
return }
    if (!selectedPhoto) { toast({ title: "Nenhuma foto selecionada", description: "Selecione uma foto." }); 
return }
    ...
    try {
      ...
      toast({ title: "Foto enviada", description: "A foto foi enviada com 
sucesso." })
 ...
 const handleUploadIndividualMaterial = async (event: React.FormEvent<HTMLFormElement>) => {
    event.preventDefault()
    if (!user?.email) { toast({ title: "Erro de autenticação", description: "Usuário não autenticado." }); 
return }
    const file = selectedIndividualPDF
    if (!file) { toast({ title: "Nenhum arquivo selecionado", description: "Selecione um PDF." }); 
return }
    if (!nomeMaterialIndividual.trim()) { toast({ title: "Erro", description: "Informe o nome do material." }); 
return }
...
`pacientes/${id}/materiais_individuais/${materialToDelete.nome}`));
        } catch (storageError) {
            console.error("Erro ao excluir do Storage, pode já ter sido removido:", storageError);
        }

        setPatient((prev: any) => {
            if (!prev) return null;
            const updatedMaterials = (prev.materiaisIndividuais || []).filter(
                (m: any) => m.url !== materialToDelete.url
            );
            return { ...prev, materiaisIndividuais: updatedMaterials };
        });

        toast({
            title: "Material individual excluído",
            description: "O material foi removido com sucesso.",
        });
    } catch (error) {
        console.error(error);
        toast({
            title: "Erro ao excluir material individual",
...
if (!user?.email) return
  ...
  const handleSaveMetrics = async () => {
    if (!user?.email) return
    const 
refp = doc(db, "nutricionistas", user.email, "pacientes", id)
    await updateDoc(refp, {
            ...
            <div className="flex items-center justify-between mb-6">
             ...
             <Button variant="outline" size="icon" asChild>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <Label htmlFor="patient-status">{isActive ? 
"Paciente Ativo" : "Paciente Inativo"}</Label>
                </div>
...
Isso removerá permanentemente o paciente e todos os seus dados
                        do Firestore.
</AlertDialogDescription>
                    </AlertDialogHeader>
                    <AlertDialogFooter>
...
"-"}</p>
                ...
                <div>
                  ...
                  <p>{patient?.telefone ? 
formatTelefone(patient.telefone) : "-"}</p>
                </div>
                <div>
...
<DialogTitle>Informações Pessoais</DialogTitle>
                ...
                <div className="space-y-4">
                  <div className="grid gap-1">
                    <Label>Nome</Label>
                    <Input value={editData.name} onChange={(e) => setEditData({ ...editData, 
name: e.target.value })} />
                  </div>
                  <div className="grid gap-1">
                    <Label>Email</Label>
...
`(${match[1]}) ${match[2]}-${match[3]}` : onlyNumbers
   ...
   />
                          {erroNomeDieta && (
                            <p className="text-sm text-red-600 mt-1">
                              Por favor, insira o nome da dieta antes 
de enviar.
                            </p>
                          )}
                        </div>
...
isSubmittingDiet}
    ...
    <div
                           ...
                           <div className="flex items-center gap-4">
                                ...
                                <div className="flex items-center gap-2">
                                    <p className="font-medium">{dieta.nomeDieta || 
dieta.nome}</p>
                                    {isUltima && (
         ...
         {patient?.fotos?.length > 0 && (
                  <Card className="mt-6">
                    <CardHeader>
                      ...
                      <CardDescription>Visualize e 
gerencie as fotos do paciente.</CardDescription>
                    </CardHeader>
                    <CardContent>
...
justify-center w-full">
                            <label
                              ...
                              className="flex flex-col items-center justify-center w-full h-32 border-2 border-dashed rounded-lg cursor-pointer bg-muted 
hover:bg-muted/80"
                            >
                              <div className="flex flex-col items-center justify-center pt-5 pb-6">
...
text-muted-foreground" />
 ...
 const file = e.target.files?.[0]
                        ...
                        {selectedIndividualPDF && (
                          <p 
className="text-sm text-green-600">{selectedIndividualPDF.name}</p>
                        )}
  ...
  disabled={!selectedIndividualPDF || isSubmittingIndividualMaterial}
        ...
        <p className="font-medium">{material.nomeMaterial || 
material.nome}</p>
                                <p className="text-sm text-muted-foreground">
   ...
   <AlertDialogHeader>
              ...
              Tem certeza que deseja excluir o material{" "}
                                      <strong>{material.nomeMaterial || 
material.nome}</strong>?
                                    </AlertDialogDescription>
                                  </AlertDialogHeader>
                                  <AlertDialogFooter>
             ...
             <th className="p-2 text-left">Indicador</th>
                   ...
                   {m.data && !isNaN(new Date(m.data).getTime())
                          ...
                          <tbody>
                            {[
                              { k: 
"gorduraPercentual", label: "% Gordura" },
...
m?.[row.k] == null || m?.[row.k] === "" ? "-" : m[row.k]}
  ...
  ? new Date(m.data).toLocaleDateString("pt-BR")
                          ...
                          {[
                                ...
                                { k: 
"densidadeCorporal", label: "Densidade Corporal (g/mL)" },
                              ].map((row) => (
                                <tr key={row.k} className="border-b hover:bg-muted/40">
...
m?.[row.k] == null || m?.[row.k] === "" ? "-" : m[row.k]}
                ...
                {isClient && metricas.length > 0 && (
                 ...
                 <Card className="mb-8">
                    ...
                    <CardContent>
                      {/* limita a largura p/ evitar gráfico achatado; 
centraliza e dá minWidth p/ 1 ponto */}
...
<CardTitle>Nova Medição</CardTitle>
           ...
           <div className="flex flex-wrap items-center gap-6 mb-6">
                      <div className="flex items-center gap-2">
                        <Switch id="sw-antrop" checked={true} onCheckedChange={(v)=>{ /* visual apenas; 
antrop aparece abaixo */ }} disabled />
...
<Label>Quadril (cm)</Label>
    ...
    <div className="flex flex-wrap gap-2">
                            {(Object.keys(PROTO_LABEL) as ProtocoloKey[]).map((p) => (
                                <Button
                                    key={p}
                                    
      variant={protocoloDobras === p ? 
"default" : "outline"}
                                    onClick={() => setProtocoloDobras(p)}
                                    size="sm"
...
>
                          ...
                          {/* Campos de dobras conforme protocolo (apenas inputs; 
nada “calculado” visível) */}
                          {(() => {
...
n(dobraTricipital) + n(dobraSupraIliaca) + n(dobraCoxa)
   ...
   n(dobraSubescapular) +
              ...
              n(dobraCoxa)
                                  densidadeCalc = densidadePollock7(somatorioDobrasCalc, getIdade(), sexoAvaliaco)
...
495 / (perc + 450) : 0
                              ...
                              // Bioimpedância 
/ percentuais (se informados)
                              const gPctManual = parseNumber(gorduraPercentualNovoInput)
                              const mgManual = massaGorduraNovo ? 
Number(massaGorduraNovo.replace(",", ".")) : undefined
                              const mlgManual = massaLivreGorduraNovo ? 
Number(massaLivreGorduraNovo.replace(",", ".")) : undefined
                              const mgPercManual = massaGorduraPercentNovo ? 
Number(massaGorduraPercentNovo.replace(",", ".")) : undefined
                              const mlgPercManual = massaLivreGorduraPercentNovo ? 
Number(massaLivreGorduraPercentNovo.replace(",", ".")) : undefined
...
undefined,
                                rcq: rcq || 
undefined,
                                riscoRcq: classifyRCQ(rcq, sexoAvaliacao) || 
undefined,
                                cmb: cmb || 
undefined,
                                classificacaoCmb: classifyCMB(cmb) || 
undefined,
                                // antropometria
                                somatorioDobras: somatorioDobrasCalc || 
undefined,
                                densidadeCorporal: densidadeCalc || 
undefined,
                                // bioimpedância/manuais
                                gorduraPercentual: gPctManual || 
undefined,
                                massaGordura: mgManual || 
undefined,
                                massaLivreGordura: mlgManual || 
undefined,
                                massaGorduraPercent: mgPercManual || 
(gPctManual ? gPctManual : undefined),
                                massaLivreGorduraPercent:
                                  mlgPercManual || 
(gPctManual ? Math.max(0, 100 - gPctManual) : undefined),
                              }
...
const refp = doc(db, "nutricionistas", user.email, "pacientes", id)
                                ...
                                const hist: any[] = snap.exists() ? 
(snap.data().historicoMetricas || []) : []
...
// =========================================================
// Definição Provisória do Componente SidebarLinks
// SUBSTITUA PELO SEU COMPONENTE REAL
// =========================================================
const SidebarLinks = ({ pathname, t }: { pathname: string, t: (key: string) => string }) => {
  const links = [
